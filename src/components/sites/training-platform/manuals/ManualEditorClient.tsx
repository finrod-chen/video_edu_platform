...
import { StepCard, type StepMediaUpdate } from "./StepCard";
import { ManualQuizLink } from "./ManualQuizLink";
import { ManualAttachments } from "./ManualAttachments";
...
export function ManualEditorClient({
  ...
  function handleStepUploaded(stepId: string, media: StepMediaUpdate) {
    setSteps((prev) =>
      prev.map((s) =>
        s.id === stepId
          ? {
    ...
    <div className="space-y-6">
      <div className="rounded-xl border border-app-border bg-white p-6">
        <div className="mb-4 flex items-center justify-between">
          <span className="rounded-full bg-app-bg px-3 py-1 text-xs font-medium text-[#5B6270]">
            {STATUS_LABEL[status]}
            {savingMeta && "・儲存中…"}
          </span>
            ...
            {status !== "trashed" ? (
              <>
                <button
                  type="button"
                  onClick={handleTrash}
                  className="rounded-lg border border-app-border px-4 py-2 text-sm text-[#5B6270] hover:bg-app-bg"
                >
                  移到垃圾桶
                </button>
                <button
        ...
        <input
          ...
          className="mb-4 w-full rounded-lg border border-app-border px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand/40"
        />
        ...
        <select
          ...
          className="mb-4 w-full max-w-xs rounded-lg border border-app-border px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand/40"
        >
        ...
        <div className="mb-2 flex flex-wrap gap-2">
          {tags.map((tag) => (
            <span
              key={tag.id}
              className="flex items-center gap-1 rounded-full bg-app-bg px-3 py-1 text-xs text-[#2B2C2F]"
            >
              {tag.name}
              <button
                type="button"
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="新增標籤"
            className="w-48 rounded-lg border border-app-border px-3 py-1.5 text-sm placeholder:text-[#B0B6C0] focus:outline-none focus:ring-2 focus:ring-brand/40"
          />
          <button
            type="button"
      ...
      <ManualAttachments manualId={manual.id} initialAttachments={initialAttachments} />
      ...
      <div>
        ...
        <button
          ...
          className="mt-3 flex items-center gap-1.5 rounded-lg border border-dashed border-app-border px-4 py-2.5 text-sm text-[#5B6270] hover:border-brand hover:text-brand"
        >
