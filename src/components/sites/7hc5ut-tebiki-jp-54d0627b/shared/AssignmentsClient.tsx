...
import type { TebikiAssignment, TebikiManual, TebikiUser } from "@/types/tebiki";
...
export function AssignmentsClient({
  ...
  assignments: TebikiAssignment[];
  manuals: TebikiManual[];
  employees: TebikiUser[];
}) {
  ...
  return (
    <>
      ...
      {adding && (
        <div className="mb-4 space-y-4 rounded-xl border border-tebiki-border bg-white p-6">
          <div>
            ...
            <select
              ...
              className="w-full max-w-md rounded-lg border border-tebiki-border px-3 py-2 text-sm"
            >
          ...
          <div>
            ...
            <div className="max-h-48 max-w-md space-y-1 overflow-y-auto rounded-lg border border-tebiki-border p-2">
              {employees.map((e) => (
                <label key={e.id} className="flex items-center gap-2 rounded px-2 py-1.5 text-sm hover:bg-tebiki-bg">
                  <input
                    type="checkbox"
          ...
          <div className="flex max-w-md gap-4">
            <div className="flex-1">
              ...
              <input
                ...
                className="w-full rounded-lg border border-tebiki-border px-3 py-2 text-sm"
              />
            </div>
          </div>
      ...
      <div className="rounded-xl border border-tebiki-border bg-white">
        {assignments.length === 0 ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {assignments.map((a) => (
                <tr key={a.id} className="border-t border-tebiki-border">
