...
import type { TebikiQuiz, TebikiQuizAttempt, TebikiQuizQuestion } from "@/types/tebiki";
...
export function QuizTakeClient({
  ...
  quiz: TebikiQuiz;
  questions: TebikiQuizQuestion[];
  lastAttempt: TebikiQuizAttempt | null;
}) {
    ...
    return (
      <div className="space-y-4">
        ...
        <button
          ...
          className="rounded-lg border border-tebiki-border px-5 py-2.5 text-sm font-medium text-[#2B2C2F] hover:bg-tebiki-bg"
        >
          重新作答
        </button>
      </div>
  ...
  return (
    <div className="space-y-4">
      {questions.map((q, index) => (
        <div key={q.id} className="rounded-xl border border-tebiki-border bg-white p-6">
          ...
          <div className="space-y-2">
            {q.choices.map((c) => (
              <label
                key={c.id}
                className="flex items-center gap-2 rounded-lg border border-tebiki-border px-3 py-2 text-sm hover:bg-tebiki-bg"
              >
                <input
                  type="radio"
                  name={`question-${q.id}`}
